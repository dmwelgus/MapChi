...
If you use geo_batch() you will quickly discover that the Census API is not perfect. Fortunately, it tends to fail in predictable ways. In my experience, there are two ways the Census API can fail. 
...
Finally, if a batch is successfully geocoded, you will see the following message: "Success!!!, n more to go". 

The geo_batch() function will keep sending a block of addresses until it is successfully geocoded. Each time a block of addresses is sent you will see one of the three messages mentioned above. The success rate can vary from 0-100% depending (I assume) on the amount of traffic the server is handling. If you see six or more failures in a row you might want to push "Stop" and try again later. 
...
Sometimes you just want to know what region an address is in. In this case all you need to do is geocode the address (see above) and then use the get_regions() function. The function takes a data frame and returns the same data frame with 1+ region variables added. 
